...
use print_error;
use print_error_return;
use return_error;

pub fn create_callbacks(builder: &Builder) -> Result<(), String> {
    ...
    return_error!(create_sink_list(&tree_view));
    ...
    // open dialog window
    {
        ...
        new_sink.connect_clicked(move |_| {
            let entries = match get_all_sinks() {
                Ok(sinks) => sinks,
                Err(err) => {
                    println!("{}", err);
                    Vec::new()
    ...
    // create new sink
    {
        ...
        create_sink.connect_clicked(move |_| {
            ...
            if null_sink.get_active() {
                ...
                let first_slave_id = match Entry::find_id_by_name(&entries, &first_slave_name) {
                    Some(id) => id,
                    None => {
                ...
                let second_slave_id = match Entry::find_id_by_name(&entries, &second_slave_name) {
                    Some(id) => id,
                    None => {
